//
//  MatchNotificationViewController.m
//  Coolio
//
...
#import "MatchNotificationViewController.h"

@interface MatchNotificationViewController ()

@end

@implementation MatchNotificationViewController
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(toNotifyTheGetMatch:) name:@"ToNotifyTheGetMyMatch" object:nil];
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if( [AppDelegate NetworkCheckConnect] == YES){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [coolio getMyMatch:myId];
            [coolio doNoticeCountReset:myId command:@"match"];
        });
    }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary* userDict = [matchCollections[indexPath.row] mutableCopy];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
     ...
     */
    //
    //[self performSegueWithIdentifier:@"MatchToAccountDetail" sender:self];
}
...
- (void)toNotifyTheGetMatch:(NSNotification *)notification
{
    matchCollections = [[notification object] mutableCopy];
    matchCollections = [self sortArrayLastLoginTime:matchCollections];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"MatchToAccountDetail"]) {
        NSIndexPath* idxPath = self.tblView.indexPathForSelectedRow;
        NSDictionary* userInfoDict = [matchCollections[idxPath.row] mutableCopy];
