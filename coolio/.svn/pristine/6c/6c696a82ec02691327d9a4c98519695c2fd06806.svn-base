...
@synthesize tblView;
//@synthesize titleSegControl = _titleSegControl;
...
- (void)viewDidLoad
{
    ...
    /*
    CGRect titleSegRect = CGRectMake(0, 0, 200, 40);
    NSArray *arr = [NSArray arrayWithObjects:@"Alert", @"News/Recommendation", nil];
    self.titleSegControl = [[UISegmentedControl alloc] initWithFrame:titleSegRect];
    self.titleSegControl =[[UISegmentedControl alloc] initWithItems:arr];

    [self.titleSegControl setSegmentedControlStyle:UISegmentedControlStyleBar];
     
     [self.titleSegControl addTarget:self action:@selector(doSelectNotifySeg:) forControlEvents:UIControlEventValueChanged];
     self.navigationItem.titleView = self.titleSegControl;
    self.titleSegControl.selectedSegmentIndex = 0;
    */
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(toNotifyTheGetAlert:) name:@"ToNotifyTheGetAlert" object:nil];
}
...
{
    NSDictionary* _dict = alertArray[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //iconView.image = [UIImage imageNamed:@"adminIcon"]; //[[UIImage alloc]initWithData:[self pictures]];
    
    NSDictionary* _infoDict = alertArray[indexPath.row];
...
/*
-(void)doSelectNotifySeg:(UISegmentedControl *)sender
{
...
-(void)toNotifyTheGetAlert:(NSNotification *)notification
{
    alertArray = [[notification object] mutableCopy];
    ...
    if([alertArray count] == 0){
        ...
        //@"There is no current alerts."
        [_dict setValue:NSLocalizedString(@"ALERTVIEW_ZERO", nil) forKey:@"message"];
        [_dict setValue:now forKey:@"updated_at"];

        [alertArray addObject:_dict];
    }
