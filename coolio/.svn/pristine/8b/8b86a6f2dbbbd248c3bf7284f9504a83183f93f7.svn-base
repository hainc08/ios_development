//
//  FrendsNotificationViewController.m
//  Coolio
//
...
#import "FrendsNotificationViewController.h"

@interface FrendsNotificationViewController ()

@end

@implementation FrendsNotificationViewController
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(toNotifyTheGetFriendRequest:) name:@"ToNotifyTheGetMyFriendRequest" object:nil];
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if( [AppDelegate NetworkCheckConnect] == YES){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [coolio getMyFriendRequest:myId];
            [coolio doNoticeCountReset:myId command:@"friendRequest"];
        });
    }else{
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return [friendNotificationCollections count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary* userDict = [friendNotificationCollections[indexPath.row] mutableCopy];
...
- (void)toNotifyTheGetFriendRequest:(NSNotification *)notification
{
    friendNotificationCollections = [[notification object] mutableCopy];
    friendNotificationCollections = [self sortArrayLastLoginTime:friendNotificationCollections];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"FriendRequestToAccountDetail"]) {
        NSIndexPath* idxPath = self.tblView.indexPathForSelectedRow;
        NSDictionary* userInfoDict = [friendNotificationCollections[idxPath.row] mutableCopy];
