//
//  FootprintNotificationViewController.m
//  Coolio
//
...
@interface FootprintNotificationViewController ()

@end

@implementation FootprintNotificationViewController
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ToNotifyTheGetFootPrint:) name:@"ToNotifyTheGetMyFootPrint" object:nil];
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if( [AppDelegate NetworkCheckConnect] == YES){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [coolio getMyFootPrint:myId];
            [coolio doNoticeCountReset:myId command:@"footprint"];
        });
    }else{
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return [footPrintCollections count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel* updatedLabel = (UILabel *)[cell viewWithTag:4];
    
    imgIcon.image = nil;
    nickNameLabel.text = nil;
    contentTextView.text = nil;
    
    NSDictionary* userDict = [footPrintCollections[indexPath.row] mutableCopy];
...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"FootPrintToAccountDetail"]) {
...
- (void)ToNotifyTheGetFootPrint:(NSNotification *)notification
{
    footPrintCollections = [[notification object] mutableCopy];
    footPrintCollections = [self sortArrayLastLoginTime:footPrintCollections];
    //if([footPrintCollections count] != 0)
    //if(![footPrintCollections isEqual:[NSNull null]]){
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"FootPrintToAccountDetail"]) {
        ...
        NSDictionary* userInfoDict = [footPrintCollections[idxPath.row] mutableCopy];
