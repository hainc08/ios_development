...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (_mode == KKPasscodeModeSet || _mode == KKPasscodeModeChange) {
		if (_passcodeLockOn) {
			...
			for (int i = 0; i < [[_boxes lastObject] count]; i++) {
				[boxesView addSubview:[_boxes lastObject][i]];
			}
		...
		for (int i = 0; i < [[_boxes lastObject] count]; i++) {
			[boxesConfirmView addSubview:[_boxes lastObject][i]];
		}
	...
	[self.view addSubview:_tableViews[0]];
	...
	[_textFields[[_tableViews count] - 1] setReturnKeyType:UIReturnKeyDone];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    NSNotification *n = [NSNotification notificationWithName:@"KKPasscodeDismiss" object:self];
...
- (void)incrementFailedAttemptsLabel
{
	...
	for (int i = 0; i < kPasscodeBoxesCount; i++) {
		[_boxes[_currentPanel][i] setImage:[UIImage imageNamed:@"KKPasscodeLock.bundle/box_empty.png"]];
	}
    ...
    NSDictionary *attributesDictionary = @{NSFontAttributeName: [UIFont boldSystemFontOfSize:self.isSmallLandscape ? 10.0f : 14.0f]};
    CGSize size = [_failedAttemptsLabel.text boundingRectWithSize:CGSizeMake(0, 999999.0f) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributesDictionary context:nil].size;
	...
	gradient.colors = @[(id)[[UIColor colorWithRed:0.7 green:0.05 blue:0.05 alpha:1.0] CGColor],
...
- (void)moveToNextTableView
{
	...
	UITableView *oldTableView = _tableViews[_currentPanel - 1];
	UITableView *newTableView = _tableViews[_currentPanel];
	...
	_shouldReleaseFirstResponser = YES;
	[_textFields[_currentPanel - 1] resignFirstResponder];
	_shouldReleaseFirstResponser = NO;
	[_textFields[_currentPanel] becomeFirstResponder];
}
...
{
	UITextField* textField = _textFields[_currentPanel];
...
- (BOOL)textField:(UITextField*)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString*)string
{
    ...
    for (int i = 0; i < kPasscodeBoxesCount; i++) {
        UIImageView *square = _boxes[_currentPanel][i];
        if (i < [result length]) {
