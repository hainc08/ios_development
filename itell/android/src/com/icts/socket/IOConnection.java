...
//		}
	...
	public void transportMessage(String text) {
		...
		try {
			...
			//message.setEndpoint(ItelApplication.server);
		} catch (Exception e) {
			error(new SocketIOException("Garbage from server: " + text, e));
			return;
		}
		resetTimeout();
		logger.info("< NDT" + message.getType());
		...
		case IOMessage.TYPE_MESSAGE:
			try {
				...
				IOAcknowledge ioKnowledge = remoteAcknowledge(message);
				ioCallback.onMessage(message.getData(),
						ioKnowledge);
			} catch (Exception e) {
				error(new SocketIOException(
		...
		case IOMessage.TYPE_EVENT:
			try {
				//message.setEndpoint(ItelApplication.server);
				Log.i("NDT", "Message data "+message.getData());
				...
				try {
					IOCallback ioCallback = findCallback(message);
					IOAcknowledge ioKnowledge = remoteAcknowledge(message);
					Log.i("NDT", "knowledge = null"+(ioCallback==null)+" "+(ioKnowledge==null));
					ioCallback.on(eventName,
							ioKnowledge, argsArray);
				} catch (Exception e) {
					error(new SocketIOException(
