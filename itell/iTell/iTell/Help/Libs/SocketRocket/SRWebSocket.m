...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    char *p = strstr(data, ":");
    *p = '\0';
    sessionId = [NSString stringWithFormat:@"%s", data];
    /* now we can make a more standard websocket connection */
    
    /* wss://example.com/socket.io/1/websocket/4d4f185e96a7b */
    sessionURL = [NSString stringWithFormat:@"ws://%@/socket.io/1/websocket/%@", hostURL, sessionId];
    
    NSLog(@"Websoket at: %@", sessionURL);
    
    
    /* perform the standard socketrocket stuff */
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:sessionURL]];
    _url = request.URL;
    _urlRequest = request;
    _secure = NO;
    _requestedProtocols = nil;
    [self open];
}
...
- (id)initWithSocketIO:(NSString *) baseUrl
{
    ...
    hostURL = baseUrl;
    NSURLRequest *handshakeRequest = [NSURLRequest requestWithURL:[NSURL URLWithString:
                                                                   [NSString stringWithFormat:@"http://%@/socket.io/1", baseUrl]
                                                                   ]];    
    
    NSLog(@"a  Sending handshake request to %@",  [NSString stringWithFormat:@"http://%@/socket.io/1", baseUrl]
          );
    
    
	urlConnection = [[NSURLConnection alloc] initWithRequest:handshakeRequest delegate:self];
...
- (void)open;
{
    ...
    if (_readyState != SR_CONNECTING && _inputStream != nil && _outputStream != nil) 
        return;
...
/* send a socket.io framed message should default to 5 */
- (void) sendEvent : (NSString *) name : (NSDictionary *) payload
{
    if (isSocketIO == NO) {
        return;
    }
    NSString *payloadStr = @"5:::";
    NSMutableDictionary *payloadDict = [[NSMutableDictionary alloc] init];
    [payloadDict setValue:name forKey:@"name"];
    NSMutableArray *argArray = [[NSMutableArray alloc] init ];
    [argArray addObject:payload];
    [payloadDict setValue:argArray forKey:@"args"];
    SBJsonWriter *jsonWriter = [SBJsonWriter new];
    jsonWriter.humanReadable = NO;
    NSString *jsonString = [jsonWriter stringWithObject:payloadDict];
    payloadStr = [payloadStr stringByAppendingFormat:@"%@", jsonString];
    [self send:payloadStr];
    return;
}
...
- (void) _processSocketIO : (id) message
{

    /* Message ID */
    
    NSString *frame = (NSString *)message;
    NSRange elementRange = [frame rangeOfString:@":"];
    NSString *element = [frame substringWithRange:NSMakeRange(0, elementRange.length) ];
    
    int mID = [element intValue];

    NSString *rem = [frame substringFromIndex:elementRange.location+1];

    /*  Message Type */
    elementRange = [rem rangeOfString:@":"];
    element = [rem substringWithRange:NSMakeRange(0, elementRange.length) ];
    rem = [rem substringFromIndex:elementRange.location+1];
    
    /* Message Endpoint */
    elementRange = [rem rangeOfString:@":"];
    element = [rem substringWithRange:NSMakeRange(0, elementRange.length) ];
    
    /* any extra data is define as Message Data */
    if ([rem length] > 1) {
        if (mID == SIO_EVENT) {
            NSString *messageData = [rem substringFromIndex:elementRange.length];
            SBJsonParser *jsonParser = [SBJsonParser new];
            NSDictionary *payloadData = [jsonParser objectWithString:messageData];
            dispatch_async(_callbackQueue, ^{
                [self.delegate webSocket:self didReceiveEvent: [payloadData objectForKey:@"args"] name:[payloadData objectForKey:@"name"]];
            });
        }
        return;
    }
        
    if (mID == SIO_HEARTBEAT) {
        [self send:@"2::"];
    }
