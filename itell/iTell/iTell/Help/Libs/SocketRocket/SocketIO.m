...
{   
    ...
    NSString *req = [encoded componentsJoinedByString:@":"];//[data stringByReplacingOccurrencesOfString:@"\\" withString:@""];
    ...
    if ([test count] > 0) {
        ...
        switch (idx) {
            ...
            case 5: {
                [self log:@"event"];
                if (packet.data && ![packet.data isEqualToString:@""]) { 
                    NSLog(@"PARSER DATA:%@",packet.data);

//                    NSError *e;
//                    NSDictionary *jsonParse = [NSJSONSerialization JSONObjectWithData: [packet.data dataUsingEncoding:NSUTF8StringEncoding] 
//                                                    options: NSJSONReadingMutableContainers 
//                                                      error: &e];
                    NSDictionary *jsonParse = [packet.data JSONValue];
                    NSLog(@"== %@",[packet.data JSONValue]);
                    //NSDictionary *jsonParse = [packet dataAsJSON];
                    NSLog(@"string to json: %@", jsonParse);
                    packet.name = [jsonParse objectForKey:@"name"];
                    packet.args = [jsonParse objectForKey:@"args"];
                    NSLog(@"%@ -- %@",packet.args,packet.name);
            ...
            case 6: {
                ...
                if ([pieces count] > 0) {
                    ...
                    NSLog(@"argsStr: %@",argsStr);
                    id argsData = nil;
                    if (argsStr && ![argsStr isEqualToString:@""]) {
                        argsData = [SocketIOJSONSerialization objectFromJSONData:argsStr error:nil];
                        if ([argsData count] > 0) {
...
- (id) dataAsJSON
{
    return [SocketIOJSONSerialization objectFromJSONData:(NSString*)[[self data] dataUsingEncoding:NSUTF8StringEncoding] error:nil];
}
